...
def generate_unique_salt():
    ...
    key = blake3.blake3(unique_input).digest()
    return key
...
def encrypt(keyword, data):
    ...
    initial_key = blake3.blake3(keyword + salt).digest()  # Combine and hash password and salt
...
def decrypt(keyword, secret):
    ...
    initial_key = blake3.blake3(keyword + salt).digest()  # Recreate initial key
